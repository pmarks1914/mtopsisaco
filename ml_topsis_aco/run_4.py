...
# Log top N feature indices
top_n = 10
wandb.log({"Top Feature Indices": optimal_features[:top_n].tolist()})
...
buf.seek(0)
wandb.log({"Feature Importance": wandb.Image(buf)})
plt.close()

# Compute and log confusion matrices
conf_matrices = multilabel_confusion_matrix(y_test, predictions)

# Log confusion matrix for each label (up to first 5 labels)
for i, conf_matrix in enumerate(conf_matrices[:5]):
    ...
    plt.savefig(buf, format='png')
    buf.seek(0)
    wandb.log({f"Confusion Matrix Label {i}": wandb.Image(buf)})
    plt.close()
