...
def mtopis_feature_ranking(X, y, topf):
    ...
    entropy = -np.sum((X / (X.sum(axis=0) + 1e-10)) * np.log2(X / (X.sum(axis=0) + 1e-10) + 1e-10), axis=0)
    ...
    relative_closeness = separation_worst / (separation_best + separation_worst + 1e-10)
...
def modified_aco_feature_reranking(X, y, selected_features, num_iterations=5, num_ants=5, decay_rate=0.3):
    ...
    for iteration in tqdm(range(num_iterations), desc="ACO Iterations"):
        for ant in range(num_ants):
            ...
            subset_size = random.randint(max(10, len(selected_features) // 10), len(selected_features) // 2)
